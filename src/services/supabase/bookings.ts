...
async function updateBooking(
  ...
  let datetime_of_slot;
  if (updates.booking_seated_date && updates.booking_seated_time) {
    // Format as YYYY-MM-DD HH:MM:SS for PostgreSQL timestamptz
  ...
  const { data: booking, error } = await supabase
    ...
    .update({
      ...updates,
      datetime_of_slot,
      time_slot_iso: datetime_of_slot ? 
        new Date(datetime_of_slot).toISOString() : undefined,
  ...
  console.log('Update response:', error || 'Success');
